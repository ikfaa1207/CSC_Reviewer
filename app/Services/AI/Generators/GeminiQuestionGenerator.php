...
/**
 * Google Gemini API question generator.
 * Generates high-quality CSE questions using the Gemini 2.5 Flash model.
 */
class GeminiQuestionGenerator extends BaseQuestionGenerator
{
    ...
    protected function generateFromApi(
        ...
        try {
                    ...
                    'responseSchema' => [
                        ...
                        'items' => [
                            ...
                            'properties' => [
                                ...
                                'problem_type_tag' => [
                                    'type' => 'STRING',
                                    'description' => 'A specific short tag describing the core concept/logic used'
                                ]
                            ],
            ...
            if ($response->successful()) {
                ...
                if ($text) {
                    ...
                    if (is_array($decoded)) {
                        ...
                        return $this->normalizeAndValidateQuestions($decoded);
                    }
                }
            }
            ...
            if ($response->status() === 429) {
                ...
                Setting::set('ai_last_error', 'Gemini API call failed with status ' . $response->status());
            }

            Log::warning('Gemini API response format was invalid: ' . ($response->body() ?? 'Empty response'));
            return null;
            
        } catch (\Exception $e) {
            Log::error('Gemini API generation failed: ' . $e->getMessage());
            Setting::set('ai_last_error', 'Gemini API generation failed: ' . $e->getMessage());
            return null;
        }
    }
    ...
    private function buildGenerationPrompt(
        ...
        $prompt = "Generate exactly {$batchCount} unique multiple-choice questions for a Philippine Civil Service Exam (CSE) self-assessment tool.\n" .
                  ...
                  "6. Return the response as a JSON array of objects matching the required schema.\n" .
    ...
    /**
     * Get category-specific formatting guidelines.
     */
        ...
        foreach ($questions as $q) {
            try {
                ...
                if (empty($q['question_text']) || empty($q['options']) || !isset($q['correct_option_index'])) {
                    Log::warning('Invalid question structure from Gemini API', ['question' => $q]);
                    continue;
                }

                // Ensure options is an array with exactly 4 items
                if (!is_array($q['options']) || count($q['options']) !== 4) {
                    Log::warning('Invalid options count from Gemini API', ['options_count' => count($q['options'] ?? [])]);
                    continue;
                }
                ...
                if ($correctIndex < 0 || $correctIndex > 3) {
                    Log::warning('Invalid correct_option_index from Gemini API', ['index' => $correctIndex]);
                    continue;
                }
